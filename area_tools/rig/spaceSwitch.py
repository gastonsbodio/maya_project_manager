...
def switchedParentMatrix(trans = None, rot = None, scale = None, switchControl = None):
	"""creates a switchable constraint betweenmultiple parent objects using matrixs nodes. 
	   ...
	   For this function to work child needs a parent node, this node will be created automatically 
       if the child is parented under root
    """
	print (switchControl)
	...
	if switchControl == None:
		switchControl = mc.circle(name = 'switch_control')[0]

	if mc.attributeQuery(ATTRB_NA, n = switchControl, ex = True) == False:
		mc.addAttr(switchControl, ln = ATTRB_NA, at = 'enum', k = True, enumName = EnumInputs)
	
	storeAttr = '{0}.'.format(switchControl) + ATTRB_NA
	...
	for p in range(0, len(parents)):
		...
		mc.connectAttr('{0}.{1}x{2}_offsetMatrix'.format(switchControl,parents[p],child),'{0}.input[{1}]'.format(choiceOMatrix,p))
	...
	for c in range(0, len(channelInputList)):
		if channelInputList[c] == None:
			pass
		elif channelInputList[c] == 'ALL':
			mc.connectAttr('{0}.output{1}'.format(decompMatrix, channelList[c][0]),'{0}.{1}'.format(child, channelList[c][1]))
		elif channelInputList[c] == 'X' or 'Y' or 'Z':
			mc.connectAttr('{0}.output{1}{2}'.format(decompMatrix, channelList[c][0], channelInputList[c]),'{0}.{1}{2}'.format(child, channelList[c][1], channelInputList[c]))
		else:
